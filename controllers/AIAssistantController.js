...
const { isABMIntent, extractABMParams } = require('../../abm/utils/abmIntentDetector');
const { formatABMChatResponse } = require('../../abm/utils/abmResponseFormatter');
const abmOrchestrationService = require('../../abm/services/ABMOrchestrationService');
class AIAssistantController {
  ...
  static async chat(req, res) {
    try {
      ...
      // ── ABM Intent: intercept company research queries ─────────────────────
      // Detect "get me insights about X", "research X company", "details on X", etc.
      // Route to ABM orchestration instead of generic AI chat.
      if (isABMIntent(message)) {
        try {
          const schema = getSchema(req);
          const { companyName, domain } = extractABMParams(message);

          logger.info('[AIAssistantController] ABM intent detected', {
            tenantId,
            companyName,
            domain,
          ...
          // Run full ABM research pipeline (Serper + Unipile + Claude)
          const abmRecord = await abmOrchestrationService.research(
            message,
            tenantId,
            schema,
            {
              ...
              domain:      domain      || null,
              unipileAccountId: null,  // will use Serper social discovery only
              forceRefresh: false,
            }
          );

          const nextBestActions = abmRecord?.metadata?.next_best_actions || [];

          // Build the DTO shape that the formatter expects
          const abmDTO = {
            company_name:        abmRecord.company_name,
            company_overview:    abmRecord.company_overview,
            industry:            abmRecord.industry,
            headquarters:        abmRecord.headquarters,
            founded_year:        abmRecord.founded_year,
            company_size_range:  abmRecord.company_size_range,
            funding_stage:       abmRecord.funding_stage,
            website:             abmRecord.website,
            social: {
              linkedin_url:        abmRecord.linkedin_url,
              linkedin_followers:  abmRecord.linkedin_followers,
              twitter_url:         abmRecord.twitter_url,
              twitter_followers:   abmRecord.twitter_followers,
              instagram_url:       abmRecord.instagram_url,
              instagram_followers: abmRecord.instagram_followers,
            },
            linkedin_posts:      abmRecord.linkedin_posts,
            recent_activities:   abmRecord.recent_activities,
            recent_funding:      abmRecord.recent_funding,
            key_decision_makers: abmRecord.key_decision_makers,
          };

          const responseText = formatABMChatResponse(abmDTO, nextBestActions);
          ...
          await AIMessageRepository.create({
            ...
            role: 'user',
            content: message,
            messageData: { intent: 'abm_research', company: companyName },
          });
          await AIMessageRepository.create({
            conversationId: conversation.id,
            tenantId,
          ...
          // ABM failed — log and fall through to generic AI chat as a soft fallback
          logger.error('[AIAssistantController] ABM research failed, falling through to AI chat', {
            error: abmErr.message,
            tenantId,
          });
        }
      }
      // ── End ABM intent block ───────────────────────────────────────────────
      ...
      if (result.assistantContext) {
        ...
        // Auto-save ICP profile when user confirms (ready_for_execution)
        // This makes the ICP available for all future campaigns via GET /profiles/latest
        if (result.readyForExecution === true && result.assistantContext.outreachType === 'outbound') {
          try {
            const ctx = result.assistantContext;
            const profileName = `ICP - ${(ctx.roles[0] || 'Prospect')} in ${(ctx.industries[0] || ctx.locations[0] || 'Market')}`;
            await ICPProfileRepository.create({
              userId,
              tenantId,
              name: profileName,
              description: ctx.icpDescription || ctx.problemStatement || null,
              icpData: {
                ...
                icpDescription: ctx.icpDescription,
                lookalikeExamples: ctx.lookalikeExamples || [],
                targetingFilters: ctx.targetingFilters || { nationality: [], seniority: [] },
              },
              searchParams: {
                intent: {
                  job_titles: ctx.roles,
                  industries: ctx.industries,
                  locations: ctx.locations,
                  keywords: ctx.problemStatement ? [ctx.problemStatement] : [],
                },
                targeting_filters: ctx.targetingFilters || {},
              },
              sourceConversationId: conversation.id,
            });
            logger.info('[AIAssistant] Auto-saved ICP profile on confirmation', { tenantId, profileName });
          } catch (saveErr) {
            logger.warn('[AIAssistant] Auto-save ICP profile failed (non-blocking)', { error: saveErr.message, tenantId });
          }
        }
      }
      res.json({
  ...
  /**
   * Get latest ICP profile for the tenant (most recently used or created).
   * Used by search and campaign pages to auto-load saved ICP settings.
   * GET /api/ai-icp-assistant/profiles/latest
   */
  static async getLatestProfile(req, res) {
    try {
      const tenantId = req.user?.tenantId;
      const profile = await ICPProfileRepository.findLatestByTenant(tenantId);
      res.json({
        success: true,
        profile: profile || null,
      });
    } catch (error) {
      logger.error('Get latest profile error', { error, tenantId: req.user?.tenantId });
      res.status(500).json({ success: false, error: 'Failed to get latest ICP profile' });
    }
  }
  /**
   * Delete ICP profile
