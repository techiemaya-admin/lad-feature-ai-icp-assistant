...
class ICPProfileRepository {
  ...
  /**
   * Get the most recently used (or most recent) ICP profile for a tenant.
   * Used to auto-load ICP settings for new searches/campaigns.
   */
  static async findLatestByTenant(tenantId) {
    const sql = `
      SELECT * FROM ai_icp_profiles
    ...
    try {
      ...
      logger.error('Repository error finding latest ICP profile', { error: error.message, tenantId });
      throw error;
    }
  }
  /**
