...
const ProspectSearchController = require('../controllers/ProspectSearchController');
...
router.post('/lead-chat', authenticateToken, async (req, res) => {
  ...
  try {
    const { message, history = [], currentTargeting = null, pendingIntent = null, conversationSummary = '', icpProfile: clientIcpProfile = null } = req.body;
    ...
    // Check if the client-sent profile has any meaningful data
    const hasClientProfile = clientIcpProfile &&
      Object.values(clientIcpProfile).some(v => typeof v === 'string' && v.trim().length > 0);

    // If client profile is empty (not yet loaded in browser), load from DB
    let icpProfile = clientIcpProfile;
    if (!hasClientProfile && tenantId) {
      try {
        ...
        if (r.rows.length) {
          ...
          icpProfile = typeof d === 'string' ? JSON.parse(d) : (d || null);
          logger.info('[lead-chat] Loaded ICP profile from DB', { tenantId, fields: Object.keys(icpProfile || {}).length });
        } else {
          logger.warn('[lead-chat] No ICP profile found in DB for tenant', { tenantId });
        }
      } catch (dbErr) {
        logger.warn('[lead-chat] Could not load ICP from DB (non-fatal)', { error: dbErr.message });
      }
    }
 ...
 * Generic intent-based prospect search.
 * Accepts a natural language query (e.g. "decision makers in hotels with
 * swimming pools in Dubai") and returns enriched prospect profiles via:
 *   Claude company discovery → Serper → Unipile → Apollo → ICP scoring
 *
 * Cost: 5 credits per call (configurable via PROSPECT_SEARCH_CREDITS env var)
 *
 * Body: { query, icpProfile?, sessionId?, seenIds?, batchSize? }
 */
router.post('/prospect-search', authenticateToken, ProspectSearchController.search);
 ...
 * Get the most recently used ICP profile for this tenant (auto-loads for search)
 */
router.get('/profiles/latest', authenticateToken, AIAssistantController.getLatestProfile);
/**
