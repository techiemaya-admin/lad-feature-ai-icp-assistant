...
class AnswerProcessorService {
  constructor() {
    if (!aiClient.isAvailable()) {
      throw new Error('No AI API key configured. Please set ANTHROPIC_API_KEY or GEMINI_API_KEY.');
    }
  }
  /**
  ...
  async analyzeAnswer({
    ...
    try {
      ...
      const text = await aiClient.generateContent(conversationPrompt, { maxTokens: 1024 });
