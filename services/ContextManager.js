...
class ContextManager {
  ...
  static initializeContext() {
    return {
      ...
      // ── Prospecting enrichment fields (new) ────────────────────────────
      icpDescription: null,         // synthesized ICP description string
      lookalikeExamples: [],        // [{name, title, company, industry, location}]
      targetingFilters: {           // hard filter criteria
        nationality: [],            // e.g. ['Indian', 'Filipino']
        seniority: [],              // derived from roles
      },
    };
  }
  /**
  ...
  static updateContext(context, intentData, message) {
    ...
    if (intentData.nationality && Array.isArray(intentData.nationality)) {
      updated.targetingFilters = updated.targetingFilters || { nationality: [], seniority: [] };
      updated.targetingFilters.nationality = [...new Set([...(updated.targetingFilters.nationality || []), ...intentData.nationality])];
    }
    if (intentData.lookalikeExamples && Array.isArray(intentData.lookalikeExamples)) {
      updated.lookalikeExamples = [...(updated.lookalikeExamples || []), ...intentData.lookalikeExamples];
    }
