...
class StageHandlers {
  ...
  static async handleOutboundICPDiscovery(message, context, conversationHistory) {
    ...
    // Check if core ICP fields are collected
    const hasCoreData = context.problemStatement &&
    ...
    // After core data: ask nationality (optional, can skip)
    if (hasCoreData && !context._askedNationality) {
      context._askedNationality = true;
      const response = await GeminiResponseGenerator.generateResponse({
      ...
      return {
        success: true, response, text: response,
        assistantContext: context, status: 'collecting_info', readyForExecution: false,
        model: 'gemini-2.5-flash', tokensUsed: null
      };
    }

    // After nationality: ask lookalike examples (optional, can skip)
    if (hasCoreData && context._askedNationality && !context._askedLookalike) {
      context._askedLookalike = true;
      const response = await GeminiResponseGenerator.generateResponse({
        stage: 'outbound_icp_discovery',
        context,
        message,
        conversationHistory,
        questionType: 'lookalike'
      });
      return {
        success: true, response, text: response,
        assistantContext: context, status: 'collecting_info', readyForExecution: false,
    ...
    const hasAllData = hasCoreData && context._askedNationality && context._askedLookalike;
    if (hasAllData) {
      // Auto-synthesize ICP description from collected context
      if (!context.icpDescription) {
        const parts = [];
        if (context.problemStatement) parts.push(`Solving: ${context.problemStatement}`);
        if (context.roles.length > 0) parts.push(`Decision makers: ${context.roles.join(', ')}`);
        ...
        if (context.dealType) parts.push(`Deal type: ${context.dealType}`);
        if (context.targetingFilters?.nationality?.length > 0) parts.push(`Preferred nationality: ${context.targetingFilters.nationality.join(', ')}`);
        context.icpDescription = parts.join('. ');
      }
  ...
  static generateConfirmationMessage(context) {
    ...
    if (context.outreachType === 'outbound') {
      ...
      if (context.targetKnowledge === 'known') {
        ...
        if (context.targetingFilters?.nationality?.length > 0) {
          parts.push(`Preferred nationality: ${context.targetingFilters.nationality.join(', ')}`);
        }
        if (context.lookalikeExamples?.length > 0) {
          parts.push(`Lookalike examples: ${context.lookalikeExamples.map(e => `${e.name || ''} (${e.title || ''} at ${e.company || ''})`).join('; ')}`);
        }
      }
    }
